...
Personalized content is stored in <course_root>/personalized_exercises
(by default, see settings.PERSONALIZED_CONTENT_DIR).

<course_root>/personalized_exercises/<exercise_key>/ is created
when the personalized exercises are generated and holds the different instances
of the exercise. The exercise instances are generated by the course staff with a shell script
"pregenerate-exercises.sh" in the course git repo. The script runs a mooc-grader container and
calls the manage.py command "pregenerate_exercises", which generates the exercise instances to the container
path /local/grader/ex-meta/<course_key>/pregenerated/<exercise_key>/. The script then copies these exercise
instances from the container to the course git repository.
'''
import logging
import os
import random
...
def grader_pregenerated_exercises_directory_path(course, exercise):
    return os.path.join(settings.PERSONALIZED_CONTENT_PATH, course["key"], "pregenerated", exercise["key"])
...
def pregenerated_exercises_directory_path(course, exercise):
    ...
    '''
    course_pregenerated_path = os.path.join(
        settings.COURSES_PATH,
        course["key"],
        settings.PERSONALIZED_CONTENT_DIR,
        exercise["key"],
    )
    if os.path.isdir(course_pregenerated_path):
        return course_pregenerated_path
    return grader_pregenerated_exercises_directory_path(course, exercise)
...
def prepare_pregenerated_exercises_directory(course, exercise):
    ...
    '''
    pregen_dir = grader_pregenerated_exercises_directory_path(course, exercise)
